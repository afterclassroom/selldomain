...
module GoDaddyReseller
  module WAPI
    ...
    def process_request(legacy_xml_or_hash)
      #DATNT DEBUG TO FIX BUG
      cltrid = GoDaddyReseller::API.next_uid[0..50]
      #END DATNT DEBUG TO FIX BUG
	...
	# ValidateRegistration: This method is used for validating a domain's period and contacts.
    def wapi_validate_registration(domainContacts, domainDataArray)
      #DATNT DEBUG TO FIX
      uuid_hash = c.class.uuid_hash
      #END DATNT DEBUG TO FIX
      response = c.soap(:ValidateRegistration, {
    ...
    protected
      ...
      def _wapi_order(order_type_sym, order_hash)
        #BEGIN DATNT FIX BUG
        shopper = order_hash[:shopper]
        obj = {
          :acceptOrderTOS => 'agree',
          :dbpuser   => 'createNew',
          :dbppwd    => 'defgh',
          :dbpemail  => 'info@example.biz'
          }
        a_new_shopper = {
          :shopper => shopper.merge(obj)
        }
        order_hash.tap { |hs| hs.delete(:shopper) }
        my_new_hash = order_hash.merge(a_new_shopper)
        order_hash = my_new_hash

        #END DATNT FIX BUG

        mytest = GoDaddyReseller::API.next_uid[0..50]
        myhashtest = { :sCLTRID => mytest }

        response = c.soap(order_type_sym, {
          order_type_sym => {
            :_attributes => { :xmlns => 'http://wildwestdomains.com/webservices/' }
          }.update(order_hash).update(creds).update(myhashtest) }#.update(c.class.uuid_hash) }
        )
        ...
        if result['result']['code'] == '1000'
          ...
          #BEGIN: all logic condition for purpose: good error content to Rails controller
          if !result['resdata'].nil?
            if !result['resdata']['domainRegistration'].nil?
              str = result['resdata']['domainRegistration']['contact'].first['error']['displaystring']
              raise(GoDaddyResellerError.new(str))
            else
              raise(GoDaddyResellerError.new(result['resdata']['error']))
            end
          else
            raise(GoDaddyResellerError.new(result['result']['msg']))
          end
          #END: all logic condition for purpose: good error content to Rails controller
        end
      end
      ...
      def _wapi_info(info_hash, type = "standard")
        ...
        #DATNT FIX BUG BECAUSE RETURNED RESULT IS NOT THE SAME AS THIS LIB PROVIDED
        #if result['result']['code'] == '1000'
    ...
    module Util
      ...
      def self.poll_result_to_order_hash(result)
        ...
        item_results.each do |item_hash|
          ...
          #DATNT: fix but of out of range because of converting time
          item[:statuses] << { :id => item_hash['status'], :timestamp => Date.strptime(item_hash['timestamp'],"%m/%d/%Y %H:%M:%s %p").to_time }
      ...
      def self.find_by_riid_and_orderid_from_poll_result(riid, orderid, poll_result)
        puts "action find_by_riid_and_orderid_from_poll_result:: "
        puts "action find_by_riid_and_orderid_from_poll_result:: riid = #{riid}"
        puts "action find_by_riid_and_orderid_from_poll_result:: orderid = #{orderid}"
        puts "action find_by_riid_and_orderid_from_poll_result:: poll_result = #{poll_result}"
